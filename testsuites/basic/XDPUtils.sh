...
function download_pktgen_scripts(){
    ...
    if [ "${type}" = "multi" ];then
        wget https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/plain/samples/pktgen/pktgen_sample05_flow_per_thread.sh?h=v5.7.8 -O ${dir}/pktgen_sample.sh
    elif [ "${type}" = "fix" ];then
        wget https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/plain/samples/pktgen/pktgen_sample01_simple.sh?h=v5.7.8 -O ${dir}/pktgen_sample.sh
        # insert fix rate packet transferring 50Kpps
        sed -i '82i pg_set \$DEV \"ratep 50000\"' ${dir}/pktgen_sample.sh
    else
        wget https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/plain/samples/pktgen/pktgen_sample01_simple.sh?h=v5.7.8 -O ${dir}/pktgen_sample.sh
    fi
    wget https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/plain/samples/pktgen/functions.sh?h=v5.7.8 -O ${dir}/functions.sh
    wget https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/plain/samples/pktgen/parameters.sh?h=v5.7.8 -O ${dir}/parameters.sh
    chmod +x ${dir}/*.sh
}
...
function start_pktgen(){
    ...
    if [ "${cores}" = "single" ];then
        ...
        modprobe pktgen; lsmod | grep pktgen
        result=$(${startCommand})
    else
...
function start_xdpdump(){
    ...
    LogMsg "Starting xdpdump with command: ${xdpdumpCommand}"
    sh -c '${xdpdumpCommand}'
}