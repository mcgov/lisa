...
class DpdkTestpmd(Tool):
    ...
    def generate_testpmd_command(
        ...
        return (
            ...
            f"{nic_include_info} --log-level netvsc,debug --log-level eal,debug -- "
            f"--forward-mode={mode} -a --stats-period 2 "
    ...
    def _install(self) -> bool:
        ...
        if self.use_package_manager_install():
            ...
            elif isinstance(distro, Suse) and float(distro.information.release) >= 15.5:
                distro.install_packages(["dpdk22", "dpdk22-devel"])
            ...
            if isinstance(distro, Suse) and float(distro.information.release) >= 15.5:
                self._dpdk_version_info = distro.get_package_information("dpdk22")
            else:
